...
3. Проверить работу Docker, запустив простой тестовый контейнер hello-world.
4. Запустить контейнер с нашим приложением, которое отвечает на GET-запрос "Привет мир!", и убедиться, что оно запускается автоматически.
5. Создать Dockerfile для нашего приложения, предусмотреть проброс порта 1234.
6. Собрать образ Docker на основе созданного Dockerfile.
7. Запустить контейнер с приложением, проверить, что при обращении по адресу [http://localhost:1234/](http://localhost:1234/) возвращается "Привет мир!".
8. Написать docker-compose.yml для развертывания этого приложения, предусмотреть проброс порта 1234, а также возможность расширения или подключения сторонних сервисов при необходимости.
9. Запустить стек через docker-compose и проверить работу.
